...
def out(n, inp):
    return np.asarray([0.2 * np.sin(n * 0.2)])
...
class ESN():
    ...
    def proceed(self):
        ...
        almost_fire = (self.A[1, :, :] != 0)
        adjustment = 1.01 - error * self.lr * almost_fire
...
esn = ESN(K=0, N=20, L=1, dropout=.8, alpha=0.2, show_plots=True)
