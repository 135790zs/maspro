...
\section{Eligibility Propagation}
    ...
    \subsection{Learning procedure}
        ...
        \end{pmatrix}
        \end{equation}
        The presence of $z$, and its relation with the step function $H$ in the hidden state updates in Equation \ref{eq:alifV} and Equation \ref{eq:alifA} seems problematic for computing these partial derivates, because the derivative $\frac{\partial z^t_j}{\partial v^t_j}$ is nonexistent.
        ...
        \end{equation}
        where $\gamma$ is a dampening constant.
        During the refractory period, the pseudo-derivative is set to 0.

        \begin{tcolorbox}[colback=orange]
        - Plot of pseudo-derivative overlayed with voltage, adaptation and spikes.
        \vspace{7cm}
        \end{tcolorbox}
        ...
        In the TIMIT framewise phone classification task, there are $K=61$ output neurons $y^t_k$ where $k \in [1\mathrel{{.}\,{.}}\nobreak K]$.
        ...
        \begin{align}
        \Delta W &= -\eta\sum_{t'}\frac{\partial E}{\partial z^{t'}_j}e^{t'}_{ji}\\
        ...
        &= -\eta\sum_t\underbrace{\sum_kB_{jk}\left(\pi^{t'}_k - \pi^{*,t}_k\right)}_{=L^t_j}\underbrace{\sum_{t'\leq t}\kappa^{t'-t}e^{t'}_{ji}}_{\eqdef \bar{e}^t_{ji}}
        \end{align}
