...
def pulse(A=(1, 1, 1), duration=(1, 1, 10), gap=(20, 20, 40), offset=(0, 6, 2)):
    ...
    ret = np.zeros(shape=(cfg["Repeats"], cfg["N_I"]))
...
def sinusoid(A=(0.3, 0.3, 0.5), f=(4, 1, 11), phase=(0, 0, 0)):
    ...
    for n in range(cfg["N_I"]):
        for t in range(cfg["Repeats"]):
            ret[:, n] = np.sin((np.arange(cfg["Repeats"]) + phase[n]) / f[n]) * A[n] + A[n]
    return ret
...
def pulseclass(A=(1, 1, 1,), duration=(3, 3, 3,), gap=(5, 5, 5,), offset=(0, 2, 4)):
    ...
    inp = np.zeros(shape=(cfg["Repeats"], cfg["N_I"]))
    tar = np.zeros(shape=(cfg["Repeats"], cfg["N_O"]))

    for t in range(cfg["Repeats"]):
        for n in range(cfg["N_I"]):
            inp[t, n] = A[n] * ((t - offset[n]) % (duration[n] + gap[n]) < duration[n])
    for t in range(cfg["Repeats"]):  # classidx is sum of active
