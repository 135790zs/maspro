...
def plot_M(cfg, M, it, log_id, n_steps, inp_size):
    ...
    for var in plotvars:
        ...
        if var in ['z', 'p', 'pm', 't', 'correct']:
            arr = M[var][b, :n_steps[b]].cpu().numpy()
        if var == 'x':
            arr = M[var][b, :n_steps[b], :inp_size].cpu().numpy()
...
def plot_GW(W, G, cfg, n_channels, n_phones, log_id):
    ...
    shapes = {
        'bias': (1, n_phones),
        'out': (cfg["N_R"], n_phones),
        'W': (cfg["N_R"]*2, cfg["N_Rec"]*cfg["N_R"]),
        'B': (cfg["N_Rec"]*cfg["N_R"], n_phones),
    }
    rowidx = 0
